...
.. Prerequisites: None.
.. Test case: Type `add` then press the kbd:[Tab] key +
...
.. Test case: Type `add`, press the kbd:[Tab] key then the kbd:[Delete] key +
   Expected: After pressing kbd:[Tab] key, expected output will be the same as above in part b.
   After pressing kbd:[Delete] key, the field and corresponding prefix will be removed and the caret will shift to the
   next field to be filled in (i.e. the command box now looks like this: `add p/ e/ a/ $/ sub/ lvl/ r/`, with the caret
   placed after `p/`.
.. Test case: Type `add`, press the kbd:[Tab] key, shift caret to middle of a field the kbd:[Delete] key (Not Recommended) +
   Expected: After pressing kbd:[Tab] key, expected output will be the same as above in part b.
...
.. Test case: `remark 1 r/Hardworking Student` +
   Expected: Creates a remark 'Hardworking Student' for the 1st person in the list.First contact is deleted from the list.
...
.. Test case: `remark 1 r/` +
   Expected: Removes the remark for the 1st person in in the list.
.. Test case: `remark 0 r/` +
   Expected: No remark is added as the index specified is not within the range of people in the list. Error details shown in the status message.
...
.. Test case: `rate 1 r/3.3` +
   ...
   person will be displayed and the number of people who rated the person will increase by 1.
...
.. Test case: `rate 0 r/4.0` +
   Expected: No rating is added as the index specified is not within the range of people in the list.
