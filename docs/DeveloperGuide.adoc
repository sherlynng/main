...
==== Current Implementation
We made input of fields case insensitive to reduce hassles for the user. The exceptions are `address`, `price`,and `phone`.
...
All the fields utilise the `ProperCaseConverter` object in similar way, except `email`,
which is converted to all lower-case to follow the convention.
