...
==== Current Implementation
We made input of all fields case insensitive.
...
See the example below for a actual implementation found in the `Role` class. All the fields utilise the `ProperCaseConverter` object in the same way.
[source,java]
----
public class Role{
...
=== Create shortcuts for the user in filling fields

==== Current Implementation
Due to the nature of the application, the user will input certain information repetitively. For example, role must be one of student or tutor.
...
|role | `stat/t`| `stat/tutor`
|role | `stat/s`| `stat/student`
|=======================================================================

==== Model Implementation
The shortcut is first validated, then the field information is converted to its full form.
Below is the function that performs conversion in the Role class. Other classes follow the same implementation.
[source,java]
----
...
Person card. This helps to improve the readability of Person details and prevents cluttering on the side panel.|
