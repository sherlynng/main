...
image::RemarkCommandSequenceDiagram.png[width="700"]
.Sequence Diagram for `remark`

image::RemarkCommandSequenceDiagram1.png[width="700"]
...
Rating is from 0.0 (worst performing) to 5.0 (best performing) and is stored as a `double` inside `Rate` class.
Regex is used to limit user input to a maximum of 1 decimal place and any number below 0.0 or 5.0 is rejected.

We have decided to allow two types of ratings:
...
abilities. In this app, cumulative rating is calculated using the current rate `value` and `count` inside `accumulatedValue` method in `Rate` class.
Every time a new rate is added, `count` increases incrementally and only the latest rate `value` is stored.
...
image::RateCommandSequenceDiagram.png[width="700"]
.Sequence Diagram for `rate`

image::RateCommandSequenceDiagram1.png[width="700"]
...
===== Aspect: Calculation for Cumulative Rating

* **Alternative 1 (current choice):** Make use of only current rating and rate count
** Pros: Takes up minimal storage space and simple to calculate.
** Cons: May not be very accurate.
* **Alternative 2:** Store all past ratings of a person and take the average
** Pros: Produces accurate rating.
** Cons: Can potentially take up lots of storage space as there is no limit to the number of times a person can be rated.
...
Similarly, when a `Pair` is removed through the `unmatch` command, its `PairHash` is retrieved and removed from the
`Student` and `Tutor` involved.
