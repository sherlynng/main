...
User can enter prefix with only spaces as a valid input, which will be in as an unentered field, which may be unintuitive.
...
When keyboard kbd:[Delete] or kbd:[Tab] is pressed, depending on the user input in the command box, the caret position
will be shifted accordingly. Below shows the workflow when kbd:[Delete] or kbd:[Tab] is pressed.
...
then procedes to find next field to be shifted to. Below shows what happens when kbd:[Delete] is pressed when caret position is at `n/`.
...
===== Aspect: Keyboard key for kbd:[Delete]

* **Alternative 1 (current choice):** Use kbd:[Delete] button.
** Pros: Intuitive to user.
** Cons: May lead to confusion since the orignal functionality of the `DELETE` button does not work now.
* **Alternative 2:** Use other keyboard keys that users typically do not use, for example kbd:[F2].
