...
>>>>>>> v1.1_enhancement3
    ...
    public void parse_fieldsMissing_failure() {
        ...
        assertParseSuccess(parser, NAME_DESC_AMY + PHONE_DESC_AMY + EMAIL_DESC_AMY + ADDRESS_DESC_AMY
                + SUBJECT_DESC_ENGLISH + PRICE_DESC_AMY + LEVEL_DESC_ONE + STATUS_DESC_UNMATCHED,
                new AddCommand(expectedPerson));
                .withEmail(VALID_EMAIL_AMY).withAddress(VALID_ADDRESS_AMY).withPrice(PRICE_AMY)
                .withSubject(SUBJECT_ENGLISH).withLevel(LEVEL_ONE).withStatus(STATUS_UNMATCHED).withTags().build();
